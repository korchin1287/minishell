...
/*   Updated: 2020/10/01 13:57:13 by ndreadno         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_check_condition_qual(char *str, int i, int export, int flag)
{
	if (!ft_isalnum(str[i]) && str[i] != '_' && export)
		return (0);
	if (!ft_isalnum(str[i]) && str[i] != '_' &&
		str[i] != '\'' && str[i] != '\"' && export == 0)
		return (0);
	if (str[i] == '\'' && str[i] != flag)
		return (0);
	if (str[i] == '\"' && str[i] != flag)
...
int	ft_check_char_qual(t_data *data, char *str, int k, char flag)
{
	...
	while (str[--i] != ' ' && i >= 0)
	{
		if (!ft_check_condition_qual(str, i, export, flag))
			return (0);
	}
	if (ft_isalpha(str[i + 1]) == 0 && str[i + 1] != '_' && export != 0)
		return (0);
	if (!ft_isalnum(str[i + 1]) && str[i + 1] != '_' &&
		str[i + 1] != '\'' && str[i + 1] != '\"' && export == 0)
		return (0);
